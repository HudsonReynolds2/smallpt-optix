...
// PHASE 3: launch index is tile-local; offset by params.tile_origin_{x,y}
// to recover the full-image pixel coordinate.
// ---------------------------------------------------------------------------
extern "C" __global__ void __raygen__rg() {
    const uint3 idx = optixGetLaunchIndex();

    // Full-image pixel (offset by tile origin)
    const unsigned int x = idx.x + params.tile_origin_x;
    const unsigned int y = idx.y + params.tile_origin_y;

    // Image dimensions (full, NOT the tile)
    const unsigned int W = params.width;
    const unsigned int H = params.height;

    // Bounds check: when image dims are not a multiple of tile size the
    // last-row/last-col tile may extend past the image. Skip those threads.
    if (x >= W || y >= H) return;

    // Seed: combine pixel index and subframe for decorrelated samples.
    unsigned int seed = (y * W + x) * 1973u + params.subframe_index * 9277u;
    lcg(seed); // warm up
    ...
    for (unsigned int s = 0; s < spp; ++s) {
        ...
        float fx = ((float)x + 0.5f + dx) / (float)W - 0.5f;
        float fy = ((float)y + 0.5f + dy) / (float)H - 0.5f;
    ...
    unsigned int pixel = (H - 1 - y) * W + x; // flip Y to match PPM convention
...
// ---------------------------------------------------------------------------
// Miss shader - background is black
// ---------------------------------------------------------------------------
...
// PHASE 3: triangles only (spheres got tessellated). The sphere branch
// is gone, which removes a divergent path inside closest-hit.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Closest-hit shader.
// RR matches phase 2 exactly: depth>4, max-of-albedo continuation prob.
// ---------------------------------------------------------------------------
extern "C" __global__ void __closesthit__ch() {
    ...
    // Russian roulette after depth 4 (preserved from phase 2)
    if (prd->depth > 4) {
